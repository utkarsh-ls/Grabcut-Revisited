...
class Graph:
    ...
    def set_edges(self, img, mask, fg_gmm: GMM, bg_gmm:GMM):
        ...
        def len_check():
            assert len(edges) == len(weights), f"e_len: {len(edges)}, w_len: {len(weights)}"
            assert len(edges) == exp_length, f"e_len: {len(edges)}, exp_len: {exp_length}"
        ...
        print("lens")
        print(len(sure_fg_nodes))
        print(len(sure_bg_nodes))
        print(len(pr_nodes))
...
def GrabCut(img, init_mask=None, rect=None, max_itrs=1):
    ...
    for _ in range(max_itrs):
        ...
        print(4)
        mask[pr_mask] = np.where(np.isin(pr_ids, fg_ids), MaskValues.pr_fg, MaskValues.pr_bg)
        print("4.5")
